interface SkeletonCardProps {
  ...
  const SkeletonCardGrid: React.FC<SkeletonCardProps> = ({ count }) => {
    return (
      <>
        {Array.from({ length: count }).map((_, index) => (
          <div
            className="relative w-full h-96 rounded-2xl overflow-hidden shadow-lg flex-shrink-0 animate-pulse"
            key={index}
          >
