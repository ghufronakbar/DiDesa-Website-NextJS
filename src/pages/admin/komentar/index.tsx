        ...
        <LayoutDashboard>
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {isLoading || isFetching ? <LoadingTable colSpan={6} count={5} />
                                    : isError ? <ErrorTable colSpan={6} />
                                        :
                                        data && data?.data?.map((item: Komentar, index: number): JSX.Element => (
                                            <tr key={index}>
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    ...
                                                    <div className='text-gray-500 text-sm'>{formatString(item.berita.judul, 25)}</div>
                                                </td>
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                    <button className="text-indigo-600 hover:text-indigo-900 mr-4" onClick={() => { setIsContentOpen(true); setContent({ ...content, judul: item.berita.judul, isi: item.isi, nama: item.warga.namaLengkap, gambar: item.berita.gambar, tanggal: formatDate(item.tanggal) }) }}>Detail</button>
                                                    <button className="text-red-600 hover:text-red-900" onClick={() => { setIsModalOpen(true); setKomentarId(item.komentarId) }}>Hapus</button>
                                                </td>
                                            </tr>
                                        ))}
                            </tbody>
            ...
            {isModalOpen && (
                <ModalConfirmation
                    ...
                    onConfirm={() => { handleDelete(komentarId) }}
                    onClose={() => { setIsModalOpen(false); setKomentarId(0) }}
                />
            )}
            {isContentOpen && (
                <ModalContent
                    title='Detail Komentar'
                    onClose={() => { setIsContentOpen(false); setContent({ nama: '', judul: '', isi: '', gambar: '', tanggal: '' }) }}
                    content={<div className='flex flex-col gap-1'>
                        <Image src={content.gambar} width={300} height={300} alt="warga" className='w-full h-40 object-cover rounded-lg' />
                        ...
                        <div className='text-black italic text-xs mt-4'>{content.tanggal}</div>
                    </div>
                    }
                />
            )}
