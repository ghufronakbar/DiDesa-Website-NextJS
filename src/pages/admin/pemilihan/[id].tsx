...
import {
  getPemilihanById,
  editPemilihan,
} from "@/services/admin/pemilihanKetua";
...
import {
  ...
  deleteCalon,
  editCalon,
} from "@/services/admin/calonKetua";
...
const EditPemilihanPage: React.FC = () => {
  ...
  const handleCalon = async () => {
    ...
    try {
      if (calon.calonKetuaId === 0) {
        const response = await createCalon(
          calon.wargaId,
          Number(id),
          calon.deskripsi
        );
  ...
  const handleConfirm = async () => {
    ...
    if (
      !pemilihan.judul ||
      !pemilihan.deskripsi ||
      !pemilihan.tanggalMulai ||
      !pemilihan.tanggalSelesai
    ) {
    ...
    try {
      const response = await editPemilihan(
        Number(id),
        pemilihan.judul,
        pemilihan.deskripsi,
        formatConvertToIsoString(pemilihan.tanggalMulai),
        formatConvertToIsoString(pemilihan.tanggalSelesai)
      );
      showToast(response.message, "success");
      setIsWaiting(false);
          ...
          <div className="flex flex-col-reverse md:flex-row-reverse border rounded-md p-4 md:p-8 w-full mx-auto justify-between gap-8">
            <div className="w-full flex gap-2 flex-col">
              ...
              {data?.data?.calonKetua.length === 0 ? (
                <p className="text-gray-500">Belum ada calon ketua</p>
              ) : null}
              {data?.data?.calonKetua.map((item: CalonKetua) => (
                <div
                  className="flex border px-4 py-2 rounded-lg w-full "
                  key={item.calonKetuaId}
                >
                  <div className="flex flex-row gap-6 items-center py-2 w-full ">
                    <Image
                      src={item.warga.foto}
                      alt={item.warga.namaLengkap}
                      width={200}
                      height={200}
                          ...
                          <FaUserEdit
                            ...
                            onClick={() => {
                              ...
                              setCalon({
                                ...
                                wargaId: item.warga.wargaId,
                                namaWarga: item.warga.namaLengkap,
                              });
                            }}
                          />{" "}
                          <MdDelete
                            ...
                            onClick={() => {
                              ...
                              setCalonKetua({
                                calonKetuaId: item.calonKetuaId,
                                namaCalonKetua: item.warga.namaLengkap,
                              });
                            }}
                          />{" "}
                        </div>
              ...
              {data &&
                data.data.calonKetua.length > 0 &&
                data.data.status !== "Belum Dimulai" && (
                  <PieChartPemilihan dataCalon={data?.data?.calonKetua} />
                )}
            </div>
            <div className="flex flex-col gap-4 w-full">
              ...
              <div className="mb-4 md:mb-0 md:flex-1">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setPemilihan({ ...pemilihan, judul: e.target.value })
                  }
                  disabled={
                    data?.data?.status !== "Belum Dimulai" ? true : false
                  }
                />
              </div>
              <div className="flex-1">
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setPemilihan({ ...pemilihan, deskripsi: e.target.value })
                  }
                  disabled={
                    data?.data?.status !== "Belum Dimulai" ? true : false
                  }
              ...
              <div className="flex-1">
                ...
                <input
                  ...
                  type="date"
                  onChange={(e) =>
                    setPemilihan({ ...pemilihan, tanggalMulai: e.target.value })
                  }
                  disabled={
                    data?.data?.status !== "Belum Dimulai" ? true : false
                  }
        ...
        <ModalContent
          ...
          content={
            <div className="flex flex-col gap-2">
              ...
              {calon.calonKetuaId === 0 ? (
                <select
                  className="w-full p-2 border border-gray-300 rounded-md"
                  value={calon.wargaId}
                  onChange={(e) =>
                    setCalon({ ...calon, wargaId: parseInt(e.target.value) })
                  }
                >
              ...
              <textarea
                ...
                onChange={(e) =>
                  setCalon({ ...calon, deskripsi: e.target.value })
                }
              />
              <button
              ...
              >
                {calon.calonKetuaId === 0 ? "Tambah" : "Simpan"}
              </button>
            </div>
          }
        />
