...
const PemilihanPage: React.FC = () => {
  ...
  const handleAdd = async () => {
    ...
    setIsWaiting(true);
    try {
      const response = await createPemilihan(
        form.judul,
        form.deskripsi,
        formatConvertToIsoString(form.tanggalMulai),
        formatConvertToIsoString(form.tanggalSelesai)
      );
      showToast(response?.message, "success");
      setIsWaiting(false);
    ...
    <LayoutDashboard>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {isLoading || isFetching ? (
                  ...
                  data &&
                  data?.data?.map(
                    (item: PemilihanKetua, index: number): JSX.Element => (
                      <tr key={index}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          ...
                          <div className="text-gray-500 text-sm">
                            {formatString(item.deskripsi, 25)}
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap ">
                          {item.calonKetua.length === 0 && (
                            <p className="w-fit h-fit bg-gray-500 px-2 py-1 rounded-lg text-xs text-white self-center m-1">
                              Belum ada partisipan
                            </p>
                          )}
                          <div className="flex flex-col items-center justify-center">
                            {item.calonKetua.map(
                              (c: CalonKetua, i: number): JSX.Element => (
                                <div
                                  className="w-fit h-fit bg-primary px-2 py-1 rounded-lg text-xs text-white self-center m-1"
                                  key={i}
                                >
                                  {c.warga.namaLengkap}
                                </div>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium flex justify-center">
                          <button
                            ...
                            onClick={() => {
                              router.push(
                                `/admin/pemilihan/${item.pemilihanKetuaId}`
                              );
                            }}
                          >
                          ...
                          {item.status === "Belum Dimulai" ? (
                            <button
                              ...
                              onClick={() => {
                                setIsModalOpen(true);
                                setPemilihanKetuaId(item.pemilihanKetuaId);
                              }}
                            >
                              Hapus
                            </button>
      ...
      {isModalOpen && (
        <ModalConfirmation
          ...
          onConfirm={() => {
            handleDelete(pemilihanKetuaId);
          }}
          onClose={() => {
            setIsModalOpen(false);
            setPemilihanKetuaId(0);
          }}
        />
      )}
      {isContentOpen && (
