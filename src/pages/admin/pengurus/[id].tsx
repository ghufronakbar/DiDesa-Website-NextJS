          ...
          <div className="flex flex-col border rounded-md p-4 w-full md:max-w-[500px] lg:max-w-[700px] mx-auto">
            <div className="mb-4">
              <div className="flex flex-row gap-6 items-center py-2">
                <Image
                  src={
                    data?.data?.warga?.foto
                      ? data?.data?.warga?.foto
                      : "/images/wallapaper.jpg"
                  }
                  alt="Logo"
                  width={200}
                  height={200}
                ...
                <div className="flex flex-col justify-center">
                  <h1 className="text-4xl font-semibold  h-fit">
                    {data?.data?.warga.namaLengkap}
                  </h1>
                  <div className="flex flex-row gap-2 items-center ">
                    {isOnEdit ? (
                      <input
                        ref={inputRef}
                        className="text-lg font-semibold text-gray-700 w-36 bg-transparent border px-2 focus:outline-none"
                        value={jabatan}
                        onChange={(e) => setJabatan(e.target.value)}
                      />
                    ) : (
                      <p className="text-lg font-semibold text-gray-700 border-transparent">
                        {jabatan}
                      </p>
                    )}
                    <button
                      className={`text-xs font-medium text-white w-fit h-fit px-2 py-1 rounded-lg ${
                        data?.data?.aksesAdmin === true
                          ? "bg-green-500"
                          : "bg-red-500"
                      }`}
                      onClick={() =>
                        handleAccessAdmin(Number(id), !data?.data?.aksesAdmin)
                      }
                      disabled={
                        isWaiting || !isOnEdit || isFetching || isLoading
                      }
                    >
                      {data?.data?.aksesAdmin === true ? "Admin" : "Non Admin"}
                    </button>
                    <FaRegEdit
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex-1">
                ...
                <div className="flex">
                  ...
                  <div className="w-full px-3 py-2 border rounded-r-md">
                    {" "}
                    {data?.data?.warga?.telepon?.slice(2)}{" "}
                  </div>
                </div>
              </div>
              <button
