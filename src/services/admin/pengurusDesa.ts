...
const setJabatanPengurusDesa = async (id: number, jabatan: string) => {
  const response = await axiosInstance.put(`/api/admin/pengurus-desa/${id}`, {
    jabatan,
  });
  return response.data;
};

const createPengurusDesa = async (wargaId: number, jabatan: string) => {
  const response = await axiosInstance.post(`/api/admin/pengurus-desa`, {
    wargaId,
    jabatan,
  });
  return response.data;
